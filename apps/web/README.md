...
### 전 페이지 모바일 최적화 (Mobile First UX)

- **강의 검색 카드 레이아웃**: 모바일에서 표(Table) 대신 카드(Card) 형태의 레이아웃을 제공하여 가로 스크롤 없이 강의 정보를 한눈에 파악할 수 있도록 개선했습니다.
- **시간표 그리드 가상 스크롤**: 좁은 화면에서도 시간표가 뭉개지지 않도록 최소 너비(`600px`)를 확보하고 가로 스크롤을 도입했습니다.
- **시인성 강화**: 수업명, 교수명, 시간 등 그리드 내부 텍스트의 폰트 크기를 상향 조정하고 볼드 처리를 통해 가독성을 높였습니다.
- **설정 및 알림 UI**: 모바일 환경에 최적화된 패딩, 버튼 크기, 폼 레이아웃을 적용했습니다.

### 고정밀 시간표 이미지 저장

- **전체 영역 캡처**: 모바일 가로 스크롤 상태에서도 전체 시간표가 잘리지 않도록 콘텐츠의 실제 크기(`scrollWidth`)를 자동 계산하는 정밀 캡처 로직을 구현했습니다.
- **고해상도 지원**: `pixelRatio: 2` 설정을 통해 확대 시에도 깨지지 않는 선명한 이미지를 제공합니다.
- **스타일 최적화**: 캡처 시 배경색과 여백(40px)을 자동으로 보정하여 깔끔한 결과물을 생성합니다.
