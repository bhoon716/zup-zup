...
## 11. 강의 상세 다이얼로그 중복 트리거 및 이벤트 버블링 해결

### 문제 상황

강의 카드(SubscriptionCard 등)를 클릭하여 상세 정보를 열었으나, 다이얼로그의 바깥 배경(Overlay)을 클릭하여 닫으려고 하면 팝업이 닫히자마자 다시 열리는 무한 반복 현상이 발생했습니다.

### 원인 분석

1. **포털 이벤트 버블링**: Radix UI의 Dialog는 DOM 구조상 `body` 하단에 생성되지만, React의 이벤트 시스템상으로는 선언된 부모 컴포넌트(`Card`)로 이벤트가 버블링됩니다.
2. **구조적 결함**: `Dialog` 전체가 `Card` 내부에 위치하여, Overlay 클릭 시 발생하는 `pointer-up` 이벤트가 부모 카드인 `onClick`을 다시 트리거하는 것이 원인이었습니다.

### 해결책

1. **구조적 격리 (Structural Isolation)**: `Dialog` 컴포넌트를 `Card` 컴포넌트의 자식 노드가 아닌 **Sibling(형제) 노드**로 분리하여 물리적/논리적으로 격리했습니다.
2. **전파 방지 적용**: `DialogContent` 영역에 `e.stopPropagation()`을 추가하여 상세 정보 내부 클릭이 부모로 전달되지 않도록 방어 로직을 강화했습니다.

### 결과

팝업 외부 클릭 시 의도한 대로 한 번에 닫히며, 이벤트 충돌로 인한 UI 버그가 완전히 해결되었습니다.
