...
## 4. 계층형 교양 카테고리 동적 선택기 구현

### 문제 상황

교양 영역(Category)을 선택했을 때, 해당 영역에 속하지 않는 상세구분(Detail)까지 모두 노출되어 사용자가 잘못된 조합으로 검색을 시도하는 문제가 있었습니다.

### 해결책

백엔드의 `/api/v1/courses/categories` API를 활용하여 클라이언트 측에서 동적 필터링을 구현했습니다.

- **구현**: 사용자가 '영역'을 선택하면, 상태 변수를 통해 해당 영역에 매핑된 '상세구분' 목록만 두 번째 셀렉트 박스에 노출되도록 연동했습니다.
- **초기화 로직**: 상위 카테고리가 변경될 때 하위 선택값을 자동으로 초기화하여 논리적 불일치를 방지했습니다.
