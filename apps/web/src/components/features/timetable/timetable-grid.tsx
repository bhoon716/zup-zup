...
export function TimetableGrid({ timetable, className }: TimetableGridProps) {
  ...
  return (
    <div className={cn("relative border rounded-xl bg-background overflow-hidden shadow-sm", className)}>
      {/* Header */}
      <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b bg-muted/30 divide-x divide-border/50">
        <div className="h-10 flex items-center justify-center text-[10px] font-bold text-muted-foreground uppercase tracking-wider">시간</div>
        {DAYS.map((day) => (
          <div 
            key={day} 
            className={cn(
      ...
      <div className="relative grid grid-cols-[60px_repeat(7,1fr)] divide-x divide-border/50 min-h-[650px]">
        {/* Time Labels Column */}
        <div className="flex flex-col bg-muted/5 divide-y divide-border/30">
          {hoursArray.map((hour) => {
            const period = hour >= 9 ? hour - 8 : null;
            return (
              <div key={hour} className="flex-1 relative flex flex-col items-center justify-center py-4 group/row">
                {period !== null && (
                  <span className="text-[9px] font-bold text-primary/80 leading-none mb-1">
                    {period}교시
                  </span>
                )}
                <span className="text-[10px] tabular-nums text-muted-foreground font-medium leading-none">
              ...
              {blocks
                ...
                .map((block: RenderingBlock) => {
                  ...
                  return (
                    ...
                    >
                      <div className="font-extrabold truncate text-[11px] mb-0.5 tracking-tight">{block.title}</div>
                      {block.subTitle && <div className="text-[9px] opacity-80 truncate font-semibold mb-1">{block.subTitle}</div>}
