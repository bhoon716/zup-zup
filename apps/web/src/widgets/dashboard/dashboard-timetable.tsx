              ...
              return (
                <div key={day} className="flex flex-col border-r last:border-r-0 border-gray-100 dark:border-gray-800">
                  ...
                  <div className="flex-1 relative">
                    ...
                    {(() => {
                      const daySchedules: DashboardScheduleItem[] = [];

                      timetable?.courses?.forEach((course) => {
                        course.schedules?.filter((schedule) => isMatch(schedule.dayOfWeek)).forEach((schedule) => {
                          daySchedules.push({ 
                            id: `${course.courseKey}-${schedule.dayOfWeek}-${schedule.startTime}-${schedule.endTime}`, 
                            title: course.name,
                            professor: course.professor,
                            classroom: course.classroom,
                            startMin: toMin(schedule.startTime), 
                            endMin: toMin(schedule.endTime), 
                            type: "course",
                          });
                        });
                      });

                      timetable?.customSchedules?.filter((schedule) => isMatch(schedule.dayOfWeek)).forEach((schedule) => {
                        daySchedules.push({
                      ...
                      // 시간순 정렬 및 중복 그룹화
                      const groups: DashboardScheduleItem[][] = [];
