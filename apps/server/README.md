...
- **Runtime behavior**: `src/main/resources/logback-spring.xml` writes both console output and a rolling file appender to the host-mounted log directory.
- **Compose hint**: `docker-compose.yml` mounts `/var/log/jbnu-sugang-helper/server` into the container and sets `LOG_FILE` to the file path above.
