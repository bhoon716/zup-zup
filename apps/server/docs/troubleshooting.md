...
## 41. CourseSchedule 업데이트 시 발생하는 불필요한 DELETE 및 INSERT 쿼리 성능 Baseline 측정
...
* **DB 부하 제거**: 소요 시간은 JPA 엔티티 로딩 및 Dirty Checking으로 인해 자바 단 메모리 연산 시간이 대부분을 차지하나, 실제 DB 서버에 인가되는 **DELETE/INSERT 디스크 I/O 쓰기 트랜잭션 오버헤드가 완전히 차단**되어 대용량 배치 처리 및 동시성 환경에서의 데이터베이스 락 경합 및 커넥션 풀 병목 현상이 획기적으로 개선될 것으로 기대됩니다.
