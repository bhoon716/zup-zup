...
ALTER TABLE courses
    ADD COLUMN average_rating FLOAT NOT NULL DEFAULT 0.0 COMMENT '평균 별점';
ALTER TABLE courses
    ADD COLUMN review_count INT NOT NULL DEFAULT 0 COMMENT '리뷰 수';
...
(
    20
) NOT NULL COMMENT '분류 (BUG, SUGGESTION, OTHER)',
    title VARCHAR
(
    100
) NOT NULL,
...
(
    20
) NOT NULL DEFAULT 'PENDING' COMMENT '진행 상태 (PENDING, IN_PROGRESS, COMPLETED, REJECTED)',
    created_at DATETIME
(
    6
) NOT NULL DEFAULT CURRENT_TIMESTAMP
...
(
    ...
    INDEX idx_feedback_status_type
(
    status,
    type
),
...
(
    255
) NOT NULL COMMENT '로컬 정적 서빙용 이미지 경로',
    original_name VARCHAR
(
    255
) NOT NULL COMMENT '원본 파일명',
    created_at DATETIME
(
    6
) NOT NULL DEFAULT CURRENT_TIMESTAMP
(
    ...
    CONSTRAINT fk_attachment_feedback FOREIGN KEY
(
    feedback_id
...
(
    ...
    CONSTRAINT fk_reply_feedback FOREIGN KEY
(
    feedback_id
) REFERENCES feedbacks
(
...
(
    50
) NOT NULL COMMENT 'STATUS_CHANGE, REPLY_CREATE, REPLY_UPDATE, FEEDBACK_DELETE 등',
    target_type VARCHAR
(
    20
) NOT NULL COMMENT 'FEEDBACK, REPLY',
