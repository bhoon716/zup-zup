...
> 상태: target contract. ISSUE-124~127 구현과 운영 검증이 끝나기 전에는 아래 명령을 그대로 실행하지 않는다. `<...>` 값은 운영자가 확인해 채우며, 비밀번호·토큰·SSH 키는 이 문서나 저장소에 넣지 않는다.
...
```text
OCI_HOST=<reserved-ip-or-duckdns-hostname>
...
- Actions가 staging manifest를 서명할 수 있도록 production Environment secret `DEPLOY_MANIFEST_PRIVATE_KEY`를 등록하고, OCI에는 대응하는 공개키를 `${RELEASE_ROOT}/secrets/deploy-manifest-public.pem`으로 `root:root`, `0600` 권한으로 설치한다. 개인키는 OCI에 복사하지 않는다.
- GitHub production Environment에는 SSH 개인키·`known_hosts` fingerprint·staging manifest 서명용 `DEPLOY_MANIFEST_PRIVATE_KEY`만 저장한다. 개인키를 이미지나 OCI 파일에 복사하지 않는다.
...
| migration | 새 앱 SHA의 one-off Flyway 컨테이너 | `validate` 성공 후 `migrate`; 실패 시 DB 자동 rollback·앱 자동 재기동 금지 |
...
GitHub Actions가 PR merge 후 실행하며, production Environment와 concurrency를 사용한다. 운영자는 실패 시 다음 단계로 억지로 진행하지 않는다.

1. **입력 검증:** `IMAGE_TAG`가 성공한 `PR CI`의 main merge commit과 동일한 정확한 40자리 SHA인지 확인한다.
...
3. **인프라 preflight:** MySQL·Redis `latest`를 pull하고 Compose healthcheck를 확인한다. 실패하면 앱 SHA pull 이후 단계로 가지 않는다. 호스트 Nginx는 `systemctl is-active nginx`와 `nginx -t`로 별도 확인한다.
4. **앱 이미지 pull:** GHCR read-only token으로 `APP_IMAGE:IMAGE_TAG`를 pull하고 ARM64 manifest와 이미지 존재를 확인한다. pull 실패 시 현재 서비스는 그대로 둔다.
5. **파일 staging:** Compose, migration, DB init, Nginx 설정을 `$STAGING_ROOT/<IMAGE_TAG>`에 SCP한다. Actions가 서명한 manifest를 root wrapper가 먼저 검증하고 staging tree를 root 소유로 잠근 뒤, checksum·파일 owner/mode·`docker compose config`·`nginx -t`를 확인하고 필요한 파일만 atomic move한다.
6. **기존 앱 중지:** 검증된 새 이미지와 파일이 준비된 뒤에만 현재 앱 컨테이너를 중지한다. MySQL·Redis volume은 삭제하거나 recreate하지 않는다.
7. **Flyway validate:** 새 앱 SHA로 one-off migration 컨테이너를 실행한다. checksum/history/순서 오류가 나면 즉시 종료한다.
8. **Flyway migrate:** validate가 성공한 경우에만 versioned SQL을 적용한다. 이 단계부터 DB 상태가 바뀔 수 있으므로 자동 rollback을 시도하지 않는다.
9. **앱 시작:** dependency healthcheck 통과 후 새 앱 컨테이너를 시작한다.
10. **readiness gate:** 내부 healthcheck와 `<API_HOST>` readiness를 모두 확인한다. 200이 아니거나 timeout이면 새 앱 컨테이너만 중지하고 `IMAGE_TAG`를 바꾸지 않는다.
11. **상태 갱신:** readiness 성공 뒤 `.env.release`를 임시 파일에 작성하고 checksum 확인 후 atomic move한다. 기록에는 이전/새 SHA를 남긴다.
12. **cleanup:** `.release-history`에 성공 배포·rollback 순서를 기록하고 현재 실행 release를 포함한 최근 3개 release/image와 named volume은 남긴다. 오래된 release 디렉터리와 미사용 앱 이미지만 보수적으로 정리하며 `docker system prune -af`는 실행하지 않는다.
13. **알림:** 성공/실패 모두 SHA, actor, stage, elapsed time, result를 CI와 제한된 OCI deploy log에 남기고 이메일을 보낸다.
...
- 기존 앱을 중지하지 않는다.
- 실패 원인(디스크, GHCR 인증, ARM64 manifest, MySQL/Redis health, Nginx 문법)을 확인하고 수정한다.
...
### 4.2 Flyway validate/migrate 실패
...
5. schema가 호환되고 이전 앱으로 서비스해야 하는 경우 이전 SHA rollback workflow를 실행한다. 호환되지 않는 schema면 앱을 다시 시작하지 말고 수동 복구 결정을 기록한다.
6. 해결 후 `validate` → `migrate` → readiness 순서로 재시도하고, 결과를 deploy log에 남긴다.
...
3. GitHub Actions `workflow_dispatch`에서 `<PREVIOUS_IMAGE_TAG>`를 입력해 rollback workflow를 실행한다.
...
## 5. 수동 rollback workflow 계약

- 공식 workflow: [`.github/workflows/rollback.yml`](../../.github/workflows/rollback.yml)
- trigger: GitHub Actions `workflow_dispatch`
- input: `IMAGE_TAG=<exactly-40-hex-previous-commit-sha>`
- 권한: production Environment, 동일한 SSH known_hosts/deploy wrapper, 동일한 concurrency group
- 순서: SHA 검증 → GHCR pull/ARM64 확인 → 필요한 파일 staging 검증 → 앱 중지 → 호환 migration 확인 → 앱 시작 → readiness → 성공 시에만 `IMAGE_TAG` 갱신
- 금지: `latest` 입력, DB volume 삭제, Flyway `clean`, 자동 DB rollback, 검증 전 상태 파일 갱신

## 6. Nginx 설정·인증서 운영

1. GitHub Actions 또는 운영자가 새 설정을 `$STAGING_ROOT/nginx/<run-id>`로 전송한다.
2. root wrapper가 허용된 site 파일만 `/etc/nginx/sites-available`로 atomic move하고 symlink를 갱신한다.
...
- E2E, 중앙 로그, Slack 알림은 초기 필수 gate가 아니다.

GitHub branch protection과 production Environment의 수동 설정은 [GitHub 설정 체크리스트](./github-settings.md)를 따르고, 신규 전환·재구축 기록은 [cutover 체크리스트](./cutover-checklist.md)에 남긴다.
