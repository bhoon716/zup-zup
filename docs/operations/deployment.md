...
- Actions가 staging manifest를 서명할 수 있도록 저장소 Actions secret `DEPLOY_MANIFEST_PRIVATE_KEY`를 등록하고, OCI에는 대응하는 공개키를 `${RELEASE_ROOT}/secrets/deploy-manifest-public.pem`으로 `root:root`, `0600` 권한으로 설치한다. 개인키는 OCI에 복사하지 않는다.
- GitHub 저장소 Actions secrets에는 SSH 개인키·`known_hosts` fingerprint·staging manifest 서명용 `DEPLOY_MANIFEST_PRIVATE_KEY`만 저장한다. 개인키를 이미지나 OCI 파일에 복사하지 않는다.
...
- 권한: 저장소 Actions secrets, 동일한 SSH known_hosts/deploy wrapper, OCI server-side `flock`
