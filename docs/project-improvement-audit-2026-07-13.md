...
1. 081, 082, 090, 091, 092로 보안·탈퇴·토큰·오류 응답 경계를 먼저 고정한다.
2. 087~089로 알림 SLA, idempotency, replay, provider timeout을 계약 테스트와 함께 확정한다.
3. 098~100과 097로 single-host 장애·복구·freshness 관측을 보강한다.
4. 093~096으로 API 입력·목록·크롤러 부하를 제한한다.
5. 083~086, 095, 102~105로 관리자 UX와 품질·의존성·업로드 방어를 마무리한다.
