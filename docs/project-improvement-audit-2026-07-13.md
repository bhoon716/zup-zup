...
- 104는 Next와 `eslint-config-next` preview version을 일치시키고 README·`apps/web/package-lock.json`·Node 22 CI를 맞췄다. stable `16.2.10`이 PostCSS `8.4.31`을 직접 고정해 현재 preview는 audit-verified temporary exception이며, stable에 보안 수정이 직접 반영되면 전환한다.
