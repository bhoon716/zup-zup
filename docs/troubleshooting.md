...
- Restart behavior: Redis is intentionally ephemeral in the minimal runtime. Missing refresh registry state is rejected without minting a new token, and a Redis restart requires the app readiness gate to recover before traffic resumes.
...
- Recovery: Redis is cache/session state rather than durable business data. The current Compose contract deliberately omits a Redis volume and Redis backup/restore automation; after a restart, expired or missing session state is handled fail-closed by the application.
