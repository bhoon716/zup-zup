...
Next lint의 `window.location.href`/`assign` 8개 경고는 내부 `/login`을 Router로 전환하고 OAuth·Discord·PWA 전체 문서 이동에는 국소 예외 사유를 남겨 제거했다. Spring 쪽 Checkstyle/PMD는 0건이었고, OAuth·Redis·Spring Batch 테스트의 raw generic matcher/captor를 타입 안전 호출로 바꿔 Java unchecked 컴파일 경고도 제거했다. `npm run web:lint`, Vitest 61개 파일·183개 테스트, production build, `apps/server ./gradlew clean check --no-daemon`이 통과한다.
