...
- Restart behavior: Redis AOF와 named volume은 정상 restart/recreate 뒤 blacklist·refresh registry·Spring Session을 보존한다. host/storage 손실처럼 state를 잃는 경우에는 missing refresh registry를 새 token 발급 없이 거부하고, app readiness가 회복될 때까지 traffic을 받지 않는다.
...
- Readiness: Redis uses authenticated `PING` plus AOF enabled/last-write-status healthcheck; app startup waits for `service_healthy`. `/actuator/health/readiness` includes `readinessState`, DB, and Redis, so Redis failure returns `DOWN/503` without exposing details. The client has 2-second connect/command limits, reconnects after recovery, and rejects commands while disconnected instead of queueing stale work.
- Persistence policy: Redis stores its full `/data` directory in the Docker named volume `sugang-helper-redis-data` with AOF `appendfsync everysec`. A clean Redis/container restart preserves state. A sudden host or storage failure can lose at most about one second of acknowledged writes; a missing refresh record remains fail-closed. `appendfsync always` was not selected because every Redis write would synchronously flush storage.
- Recovery: Redis is cache/session state rather than durable business data. The current Compose contract keeps AOF state for normal restart/recreate but deliberately omits off-host backup/restore automation. Host-loss recovery remains out of scope; missing state is handled fail-closed by the application.
- Alert boundary: a failed Redis preflight blocks server deployment and uses the existing deployment Discord failure channel. Prometheus now stores app Actuator metrics for Grafana inspection; public availability remains with the external uptime monitor, and Alertmanager routing is intentionally not part of the minimal runtime.
- Evidence: `RedisConfigTest`, `RedisReadinessHealthTest`, `SecurityRequestAuthorizationTest`, and `verify-compose-policy.sh`.

### DB least privilege and durable-state disaster recovery (2026-07-13)

- Scenario: the application had access to MySQL root credentials and only logs/Loki/Alloy had a backup path. Database rows, feedback attachments, Grafana state, and proxy certificates could not be recovered as one tested point.
...
- Recovery: the current policy keeps only a server-local MySQL logical dump created by `/home/ubuntu/jbnu-sugang-helper/backup-db-local.sh`. It does not claim host-loss recovery, encrypted off-host retention, uploaded-file versioning, or a full durable-state drill. The script and systemd timer are installed directly on OCI and are intentionally outside git.
- Evidence: `migrationTest` runs the schema and account-privilege checks under the migrator contract. A server-side backup dry-run and isolated restore check are required before enabling the timer.
...
수동 실행과 주간 schedule은 `apps/web/e2e`의 Playwright Chromium smoke를 실제 Next.js 화면에서 실행해 세션 401→refresh→재시도와 관리자 feedback attachment preview를 mock API로 검증하고 `apps/web/playwright-report` artifact를 남긴다. 서버의 auth/provider/outbox/feedback 테스트는 PR Server 검증에 포함되므로 별도 주간 중복 suite를 두지 않는다. 브라우저 smoke가 실패하면 artifact의 trace/screenshot/video를 먼저 확인한다.
...
- Result: matching Next and ESLint preview versions are explicitly pinned as a temporary production exception because they include PostCSS `8.5.10`. Web now owns `apps/web/package-lock.json`, CI installs from `apps/web`, and no root npm workspace is required.
- Evidence: clean `cd apps/web && npm ci --legacy-peer-deps`, 183 Vitest tests, lint, production build, and the weekly dependency audit configuration.
- Limitation: `@emoji-mart/react` still declares React 18-only peer metadata. Lockfile updates require a reviewed `cd apps/web && npm install --package-lock-only --legacy-peer-deps`; its picker test, clean install, lint, and build remain required. The stable transition condition and weekly audit are in [web-dependency-policy.md](web-dependency-policy.md).
...
2. **Lockfile 불일치**: `apps/web/package-lock.json`의 변경 사항이 컨테이너 빌드 시점에 완전히 반영되지 않거나 캐시된 이미지를 사용하여 구버전 의존성이 유지됨.
...
2. **의존성 잠금 동기화**: `apps/web`에서 `npm install --legacy-peer-deps`를 새로 수행하여 `apps/web/package-lock.json`을 갱신하고, 이를 Docker 빌드 시 `npm ci --legacy-peer-deps` 과정에서 사용하도록 구성.
...
## 82. 웹·Spring 린트 경고 정리 (ISSUE-107)

Next lint의 `window.location.href`/`assign` 8개 경고는 내부 `/login`을 Router로 전환하고 OAuth·Discord·PWA 전체 문서 이동에는 국소 예외 사유를 남겨 제거했다. Spring 쪽 Checkstyle/PMD는 0건이었고, OAuth·Redis·Spring Batch 테스트의 raw generic matcher/captor를 타입 안전 호출로 바꿔 Java unchecked 컴파일 경고도 제거했다. `cd apps/web && npm run lint`, Vitest 61개 파일·183개 테스트, production build, `apps/server ./gradlew clean check --no-daemon`이 통과한다.
