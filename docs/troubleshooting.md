...
현재 minimal runtime은 Prometheus/Alertmanager 없이 Loki·Alloy·Grafana를 `observability` profile로 실행한다. Alloy는 Docker JSON 로그와 호스트 로그를 Loki에 전송하고, Loki는 `/var/lib/jbnu-sugang-helper/loki`에 30일 보존한다. `infra/scripts/test-observability-smoke.sh`는 compose recreate 전 해당 설정·volume·수집 경로를 검증한다. Prometheus metric과 Alertmanager webhook은 별도 후속 범위다.
