...
- `main`의 모든 merge가 production CD를 시작한다. 경로 필터로 배포를 생략하지 않는다.
- production deploy와 rollback은 GitHub Actions `concurrency`로 직렬화하며, 진행 중 작업은 취소하지 않는다.
...
- Docker Compose에는 앱·MySQL·Redis만 둔다. MySQL·Redis는 현재 합의대로 Compose `latest`를 사용하되, 매 앱 배포의 preflight에서 pull/health를 확인한다. 이 선택은 공식 인프라 rollback을 제공하지 않는 위험 수용이다.
- Nginx는 컨테이너 이미지가 아니라 OCI 호스트 패키지 reverse proxy다. 인증서는 호스트 certbot timer와 renew hook으로 갱신하고, `nginx -t` 성공 후 reload한다.

### 3. CI와 CD

PR CI는 GitHub Actions에서 MySQL·Redis service container를 시작하고 Flyway `validate`·`migrate`, 백엔드 테스트·빌드, ARM64 앱 이미지 빌드를 수행한다. E2E는 필수 merge gate가 아니며 별도 수동/주기 실행 대상으로 남긴다.
...
disk preflight
  ...
  -> 새 앱 SHA pull 및 존재/플랫폼 확인
  -> staging 파일 SCP + checksum/Compose/Nginx 검증 + atomic move
  -> 기존 앱 중지
  -> 새 SHA one-off Flyway validate
...
preflight, 파일 검증, migration, readiness 중 하나라도 실패하면 다음 단계로 가지 않는다. migration 실패 시 DB를 자동 rollback하지 않고 앱도 자동 재기동하지 않는다. readiness 실패 시 새 컨테이너만 중지하고 기존 `IMAGE_TAG`를 유지한다. 운영자는 rollback workflow에서 이전 SHA를 명시해 복구한다.
...
- deploy 사용자는 staging 디렉터리에만 쓰고, `sudo`는 SHA·경로를 검증하는 고정 wrapper(`deploy`, `rollback`, `nginx-config-apply`)만 실행한다. `eval`과 임의 문자열 shell 실행은 금지한다.
- Compose, 스크립트, Nginx 설정은 staging으로 전송한 뒤 checksum과 문법을 확인하고 atomic move한다. 반쪽짜리 설정을 운영 경로에 남기지 않는다.
...
- 외부 uptime monitor는 민감 정보를 반환하지 않는 readiness endpoint를 1분마다 확인하고, 2회 연속 실패와 복구를 이메일로 알린다. Slack과 전체 Prometheus/Grafana/Loki/Alertmanager 스택은 후속 범위다.
...
- 공식 rollback 범위는 앱 이미지뿐이다. GitHub Actions `workflow_dispatch`에서 40자리 이전 SHA를 입력한다.
...
| --- | --- | --- | --- |
| 배포 연결 | GitHub Actions → 제한된 SSH wrapper | 수동 SSH, self-hosted runner | 운영자가 매번 수동 실행하지 않아도 되고 runner를 서버에 두지 않는다. GitHub 장애 시 rollback 경로가 막히는 위험은 수용한다. |
...
| migration 실패 | 앱 중지·DB 자동 rollback 없음 | 자동 DB rollback, migration 중 기존 앱 유지 | destructive 자동 복구를 피하고 forward-fix를 선택한다. 실패 시 수동 복구 시간이 필요하다. |
| rollback | 수동 GitHub workflow | 자동 rollback, DB 포함 rollback | 잘못된 자동 판단으로 상태를 악화시키지 않는다. 운영자 대응이 필수다. |
...
- **관측 최소화:** 이메일·제한된 로그만으로 시작한다. 장애 원인 분석 요구가 커지면 외부 모니터링과 중앙 로그를 추가한다.
...
- Terraform/IaC 전면 도입
- 전체 Prometheus/Grafana/Loki/Alertmanager 및 Slack 알림
