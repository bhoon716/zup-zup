...
for variable in \
  ...
  DB_RUNTIME_PASSWORD \
  DB_MIGRATOR_USER \
  DB_MIGRATOR_PASSWORD; do
  require_variable "${variable}"
done

validate_identifier "MYSQL_DATABASE" "${MYSQL_DATABASE}"
for account_variable in DB_RUNTIME_USER DB_MIGRATOR_USER; do
  validate_identifier "${account_variable}" "${!account_variable}"
done

if [ "${DB_RUNTIME_USER}" = "root" ] \
  || [ "${DB_MIGRATOR_USER}" = "root" ]; then
  echo "service accounts must not use the root username" >&2
  exit 1
fi

if [ "${DB_RUNTIME_USER}" = "${DB_MIGRATOR_USER}" ]; then
  echo "runtime and migrator usernames must be distinct" >&2
  exit 1
fi
