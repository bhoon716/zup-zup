...
require("SERVER_HOST" in cd and "SERVER_USER" in cd and "SERVER_DOTENV" in cd, "CD must use the existing server secrets")
require("OCI_HOST" not in cd and "OCI_KNOWN_HOSTS" not in cd and "DEPLOY_MANIFEST_PRIVATE_KEY" not in cd, "CD must not require OCI-specific extra secrets")
